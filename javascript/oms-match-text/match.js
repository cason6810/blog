...
const main = {
    ...
    readOmsFile() {
        return new Promise((resolve, reject)=>{
            fs.readFile(omsFile, function (err, data) {
                ...
                main.matchName(_omsData.typeData);
                resolve();
            })
        })
    },
    ...
    matchName(_data) {
        for (let key in _data) {
            for (let val in main.params.configFile) {
                if (_data[key].title === val) {
                    _data[key].width = main.params.configFile[val];
                }
            }
        }
    ...
    writeText(_array) {
        // 格式化一下
        let _temp = jsBeautify(JSON.stringify(_array), { indent_size: 2, space_in_empty_paren: true });
